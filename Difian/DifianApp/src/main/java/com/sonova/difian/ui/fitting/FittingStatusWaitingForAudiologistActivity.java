...
public final class FittingStatusWaitingForAudiologistActivity extends Activity implements FittingServiceConnectionFragmentCallback
{
    private static final String UI_LOCKING_FRAGMENT_TAG = FittingStatusWaitingForAudiologistActivity.class.getName() + ">0";
    private static final String FITTING_SERVICE_CONNECTION_FRAGMENT_TAG = FittingStatusWaitingForAudiologistActivity.class.getName() + ">1";
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.com_sonova_difian_ui_fitting_fittingstatuswaitingforaudiologistactivity);
    ...
    public void onFittingServiceStateUpdated(FittingConnectionState state, FittingConnectionError error, String id, String audiologistName, Bitmap audiologistPicture, HiMuteStatus muteStatusLeft, HiMuteStatus muteStatusRight)
    {
        if (!_uiLocking.isUiLocked())
        {
            if (!((state == FittingConnectionState.CONNECTING_WAITING) && (id != null) && (audiologistName == null) && (error == FittingConnectionError.NONE) && (_fittingServiceConnection.getBinder().getChatMessageCount() == 0)))
            {
                _uiLocking.lockUi();
                setResult(RESULT_OK);
                finish();
